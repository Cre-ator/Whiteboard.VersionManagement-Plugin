...
class version_management_api
{
    ...
    public static function set_temp_version_name ( $version_ids )
    {
        for ( $version_index = 0; $version_index < count ( $version_ids ); $version_index++ )
        {
            ...
            /** update version name */
            $version_object->update_version_db_value ( 'version', $temp_version_name );
    ...
    public static function set_version_data ( $version_ids, $version_names )
    {
        ...
        $post_version_document_types = $_POST[ 'version-type' ];
        ...
        for ( $version_index = 0; $version_index < count ( $version_ids ); $version_index++ )
        {
            ...
            $version_document_type = $post_version_document_types[ $version_index ];

            $version_object = new version_object( $version_id );

            /** update version name */
            $version_object->update_version_db_value ( 'version', $version_name );

            /** update version released */
            $version_object->update_version_db_value ( 'released', $version_released );

            /** update version obsolete */
            $version_object->update_version_db_value ( 'obsolete', $version_obsolete );

            /** update version date_order */
            $version_object->update_version_db_value ( 'date_order', $version_date_order );

            /** update version description */
            $version_object->update_version_db_value ( 'description', $version_description );

            /** update version document type */
            if ( !empty( $post_version_document_types ) )
            {
                require_once ( __DIR__ . '/../../SpecManagement/core/specmanagement_database_api.php' );
                $specmanagement_database_api = new specmanagement_database_api();
                $version_project_id = $version_object->get_version_db_value ( 'project_id' );

                if ( strlen ( $version_document_type ) > 0 )
                {
                    $type_id = $specmanagement_database_api->get_type_id ( $version_document_type );
                    $specmanagement_database_api->update_version_associated_type ( $version_project_id, $version_id, $type_id );
                }
                else
                {
                    $specmanagement_database_api->update_version_associated_type ( $version_project_id, $version_id, 9999 );
                }
            }
        ...
        if ( count ( $version_names ) > count ( $version_ids ) )
        {
            for ( $new_version_index = $new_version_start_index; $new_version_index < count ( $version_names ); $new_version_index++ )
            {
                ...
                if ( strlen ( $new_version_name ) > 0 )
                {
                    ...
                    $version_object = new version_object( null );
                    $version_object->set_version_name ( $new_version_name );
                    $new_version_id = $version_object->insert_version ( $current_project );
                    $version_object->set_version_id ( $new_version_id );

                    /** update version name */
                    $version_object->update_version_db_value ( 'version', $new_version_name );

                    /** update project id */
                    $version_object->update_version_db_value ( 'project_id', $current_project );

                    /** update version date_order */
                    $version_object->update_version_db_value ( 'date_order', $new_version_date_order );

                    /** update version description */
                    $version_object->update_version_db_value ( 'description', $new_version_description );
