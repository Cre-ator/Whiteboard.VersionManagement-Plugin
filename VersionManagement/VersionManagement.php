...
class VersionManagementPlugin extends MantisPlugin
{
   ...
   function config ()
   {
      ...
      (
         'show_menu' => ON,
         'show_footer' => ON,
         'unused_version_row_color' => '#907C70',
         # plugin- and write access level
         'access_level' => ADMINISTRATOR,
         # read access level
         'r_access_level' => ADMINISTRATOR
      );
   }
   ...
   function checkUserHasReadLevel ()
   {
      $project_id = helper_get_current_project ();
      $user_id = auth_get_current_user_id ();

      return user_get_access_level ( $user_id, $project_id ) >= plugin_config_get ( 'r_access_level', ADMINISTRATOR );
   }

   function footer ()
   {
      if ( plugin_config_get ( 'show_footer' ) && $this->checkUserHasReadLevel () )
      {
   ...
   function menu ()
   {
      ...
      if ( ( !plugin_is_installed ( 'WhiteboardMenu' ) || !file_exists ( config_get_global ( 'plugin_path' ) . 'WhiteboardMenu' ) )
         && plugin_config_get ( 'show_menu' ) && $this->checkUserHasReadLevel ()
      )
      {
         return '<a href="' . plugin_page ( 'version_view_page' ) . '&amp;sort=ddesc&amp;edit=0&amp;obsolete=0">' . plugin_lang_get ( 'menu_title' ) . '</a >';
      }
      return null;
   }
